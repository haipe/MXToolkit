...
namespace DuiLib {
		...
		DWORD dwAlignment/* = eMenuAlignment_Left | eMenuAlignment_Top*/)
	{
        ...
        //适配DPI add by jshuang 2019-08-21
        CDuiRect rcClient;
	...
	LRESULT CMenuWnd::OnCreate(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
	{
		...
		if( m_pOwner != NULL) {
            ...
            m_pLayout->m_pWindow = this;//提前赋值，在初始化子控件的时候使用pWindow loki
            ...
            auto *pShadow = m_pOwner->GetManager()->GetShadow();
            ...
            //loki add 避免每次加载
            CMenuUI* pRoot = dynamic_cast<CMenuUI*>(
                m_builder.GetMarkup()->IsValid() ?
                m_builder.Create(this, &m_pm) : m_builder.Create(m_xml, UINT(0), this, &m_pm));

            if(pRoot)
                pRoot->m_pWindow = this;//提前赋值，在初始化子控件的时候使用pWindow loki
        ...
        //GetMenuUI()->m_pWindow = this; 已经提前赋值了 loki
	...
	void CMenuWnd::ResizeMenu()
	{
		...
		//begin add by jason 2018-07-26
		int nScreenWidth = GetSystemMetrics( SM_CXSCREEN );
		int nScreenHeight = GetSystemMetrics( SM_CYSCREEN );

		DWORD dwOldAlignment = m_dwAlignment;
		GetMonitorWorkArea( nScreenWidth, nScreenHeight, point );
		...
		//loki add
		if ( m_tme_wnd && m_tme_flag )
		{
			//取消原来的鼠标跟踪
			//避免菜单无法弹出
			TRACKMOUSEEVENT tme = { sizeof( tme ) };
			tme.dwFlags = TME_CANCEL | m_tme_flag;
			tme.hwndTrack = m_tme_wnd;
			TrackMouseEvent( &tme );
		}
		m_dwAlignment = dwOldAlignment;//还原
		//MoveWindow(m_hWnd, rc.left, rc.top, rc.GetWidth(), rc.GetHeight(), FALSE);
		if (rc.top < 0)//列表过长显示不全处理 Zero
		{
			rc.top += nHeight / 2 + 30;
			rc.bottom += nHeight / 2 + 30;
		}
	...
	void CMenuWnd::GetMonitorWorkArea( int &nWidth, int &nHeight, POINT &pt )
	{
		HMONITOR hMonitor = 0;
		...
		if ( hMonitor != NULL )
		{
			...
			nHeight = monitorInfo.rcMonitor.bottom;
			nWidth = monitorInfo.rcMonitor.right;
		}
	}
    
    void CMenuWnd::GetMonitorWorkArea(CDuiRect& area, HWND wnd)
    {
        HMONITOR hMonitor = MonitorFromWindow(wnd, MONITOR_DEFAULTTONULL);
        //HMONITOR hMonitor = 0;
        //hMonitor = MonitorFromPoint(pt, MONITOR_DEFAULTTONULL);
        if (hMonitor != NULL)
        {
    ...
    void CMenuWnd::ResizeSubMenu()
	{
        ...
        GetMonitorWorkArea(rcWork, m_pOwner->GetManager()->GetPaintWindow());
// #if defined(WIN32) && !defined(UNDER_CE)
// 		MONITORINFO oMonitor = {}; 
// 		oMonitor.cbSize = sizeof(oMonitor);
// 		::GetMonitorInfo(::MonitorFromWindow(*this, MONITOR_DEFAULTTOPRIMARY), &oMonitor);
...
// #endif
		...
		while( pReceiver != NULL ) {
			...
			if( pContextMenu != NULL && pContextMenu->GetHWND() == m_pOwner->GetManager()->GetPaintWindow()) 
			{/*&&后面的判断实际上导致了这一段while判断是不会让bReachRight和bReachBottom变为true的，这两个值是为了防止多级菜单会被即将生成的下一级菜单遮挡而设立的*/
        ...
        //更新basedPoint loki 
        m_BasedPoint.x = rc.left;
        m_BasedPoint.y = rc.top;
	...
	//loki add
	void CMenuWnd::SetTMEEvent( HWND hwnd, DWORD flag )
	{
		m_tme_wnd = hwnd;
		m_tme_flag = flag;
	}
	...
	LRESULT CMenuWnd::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
	{
		...
		if( !isClosing && m_pm.MessageHandler(uMsg, wParam, lParam, lRes) ) return lRes;
	...
	void CMenuElementUI::DoEvent(TEventUI& event)
	{
        ...
        {
            RECT rc = m_rcItem;
            if (!::PtInRect(&rc, event.ptMouse)) //loki add 如果鼠标不在菜单上的时候
                return;
	...
	void CMenuElementUI::CreateMenuWnd()
	{
        ...
        //loki 子菜单资源ID绑定父菜单的
        m_pWindow->m_pm.SetResourceModuleID(this->GetManager()->GetResourceModuleID());
	}
