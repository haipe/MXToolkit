...
Label_ForeImage:
	...
	{
        if (m_pManager)
            return m_pManager->GetDPIObj()->Scale(m_cxyCheckBox.cx);
	...
	{
        if (m_pManager)
            return m_pManager->GetDPIObj()->Scale(m_cxyCheckBox.cy);
	...
	LPCTSTR CListTextExtElementUI::GetText(int iIndex)
	{
		CDuiString* pText = static_cast<CDuiString*>(m_aTexts.GetAt(iIndex));
		if ( pText )
		{
			//loki 增加多语言获取缓存判断
			if ( !m_pManager || !IsResourceText() )//|| m_nLanguageIdentification == CResourceManager::GetInstance()->GetLanguage() 
