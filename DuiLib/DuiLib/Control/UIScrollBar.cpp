...
namespace DuiLib
{
	...
	void CScrollBarUI::DoEvent(TEventUI& event)
	{
		...
		if( event.Type == UIEVENT_BUTTONDOWN || event.Type == UIEVENT_DBLCLICK )
		{
			...
			//屏蔽掉，鼠标弹起时才发送消息	by jason 2018-05-17
		...
		if( event.Type == UIEVENT_BUTTONUP )
		{
			...
			//鼠标弹起时才发送消息	by jason 2018-05-17
			if( m_pManager != NULL) m_pManager->SendNotify(this, DUI_MSGTYPE_SCROLL);
			return;
		}
		if( event.Type == UIEVENT_MOUSEMOVE )
		{
			if( (m_uThumbState & UISTATE_CAPTURED) != 0 ) {
                if (!m_bHorizontal) {
                    ...
                    if (vRange != 0) m_nLastScrollOffset = (event.ptMouse.y - m_ptLastMouse.y) * m_nRange / abs(vRange);
                }
                else {
                    int hRange = m_rcItem.right - m_rcItem.left - m_rcThumb.right + m_rcThumb.left - 2 * m_cxyFixed.cy;
                    if (hRange != 0) m_nLastScrollOffset = (event.ptMouse.x - m_ptLastMouse.x) * m_nRange / abs(hRange);
                }
			}
			else {
		...
		if( event.Type == UIEVENT_TIMER && event.wParam == DEFAULT_TIMERID )
		{
			...
			if( (m_uThumbState & UISTATE_CAPTURED) != 0 ) {
				...
				//loki -减少定时器通知到UI频率---------------------begin
				if ( m_nLastScrollOffset <= 0 )
					return;
				//---------------------------end
				Invalidate();
			}
