...
namespace DuiLib {
	...
	void CComboWnd::Notify(TNotifyUI& msg)
	{
		...
		//loki combo的弹出窗口支持 Button和Option的事件通知到主--------- begin
		...
		else if (msg.sType == DUI_MSGTYPE_UNSELECTED)
		{
			m_pOwner->GetManager()->SendNotify(msg);
		}
		//----------------------------------- end
	...
	void CComboWnd::Init(CComboUI* pOwner)
	{
        ...
        if (szDrop.cx > 0)    // 计算弹出窗口宽度
        {
            if (m_pOwner->GetDropBoxAlign() == DC_LEFT)       // 下拉列表左对齐
                rc.right = rc.left + szDrop.cx;	
            else if (m_pOwner->GetDropBoxAlign() == DC_RIGHT) // 下拉列表右对齐
                rc.left = rc.right - szDrop.cx;
        }

        rc.top = rcOwner.bottom;		// 父窗口left、bottom位置作为弹出窗口起点
        ...
        //适配dpi by jason 2019-08-12 modify by jshuang 2019-09-18
        double dScale = pOwner->GetManager()->GetDPIObj()->GetScale() / 100.0f;
        cyFixed += 4 * dScale; // CVerticalLayoutUI 默认的Inset 调整
        rc.right = rc.left + rc.GetWidth() / dScale;
        if (szDrop.cy > 0)
            rc.bottom = rc.top + MIN(cyFixed, szDrop.cy) / dScale;// 计算弹出窗口高度
        else rc.bottom = rc.top + cyFixed / dScale;

		MONITORINFO oMonitor = {};
		oMonitor.cbSize = sizeof(oMonitor);
		::GetMonitorInfo(::MonitorFromWindow(pOwner->GetManager()->GetPaintWindow(),
            MONITOR_DEFAULTTOPRIMARY), &oMonitor);
		CDuiRect rcWork = oMonitor.rcWork;

        CDuiPoint pt = {rc.right, rc.bottom};
        ::ClientToScreen(pOwner->GetManager()->GetPaintWindow(), &pt);
        
		//add by jason 2018-11-08 右侧和底部坐标纠正
		bool isBeyondRight = false;
		bool isBeyondBottom = false;
        // modify by jshuang 2019-10-30
		if (pt.x > rcWork.right ) isBeyondRight = true;
		if (pt.y > rcWork.bottom ) isBeyondBottom = true;

        if (isBeyondRight)
        {
            int width = abs(rc.right - rc.left);
            rc.left = rcOwner.right - width*dScale;
            rc.right = rc.left + width;
        }
        if (isBeyondBottom)
        {
            int height = abs(rc.bottom - rc.top);
            rc.top = rcOwner.top - height*dScale;
        ...
        //dpi适配 add by jason 2019-08-12
        CDuiRect rcClient;
        ::GetWindowRect(m_hWnd, &rcClient);
        int dpi = CDPI::GetDPIOfMonitorNearestToPoint({ rcClient.left + rcClient.GetWidth() / 2, rcClient.top + rcClient.GetHeight() / 2 });
        m_pm.SetDPI(dpi);
	...
	void CComboWnd::EnsureVisible(int iIndex)
	{
		...
		if ( nCurSel < 0 || iIndex < 0 || iIndex >= m_pLayout->GetCount() ) return;
	...
	{
        // item 与combo 使用同高度
        m_ListInfo.m_cxyListFixed = m_cxyFixed; //modify by jshuang 2019-09-30
        if (m_ListInfo.m_cxyListFixed.cy > 2)
        {
            m_ListInfo.m_cxyListFixed.cy -= 2;  // cy 减2 调整高度(去除阴影像素)
        }
		return &m_ListInfo;
	}
	...
	bool CComboUI::SelectItem(int iIndex, bool bTakeFocus, bool bMsg)
	{
		...
		if( !pControl /*|| !pControl->IsEnabled()*/ ) return false;	 //bug24667,其他端主讲设置背景颜色，本端背景颜色选择下拉框选项未变化
		IListItemUI* pListItem = static_cast<IListItemUI*>(pControl->GetInterface(_T("ListItem")));
		if( pListItem == NULL ) return false;
		m_iCurSel = iIndex;
		if( m_pWindow != NULL || bTakeFocus ) pControl->SetFocus();
		pListItem->Select(true);
		if( m_pManager != NULL && bMsg ) m_pManager->SendNotify(this, DUI_MSGTYPE_ITEMSELECT, m_iCurSel, iOldSel);
		Invalidate();
	...
	//loki 保留个数参数 -----begin
	void CComboUI::RemoveAll( int reserveCount/* = 0 */)
	{
		m_iCurSel = -1;
		CContainerUI::RemoveAll( reserveCount );
	}
	//-----------------------end
    ...
    //是否激活菜单----loki
    bool CComboUI::IsActivate()
    {
        return m_pWindow != nullptr && IsWindowVisible(m_pWindow->GetHWND());
    }
    ...
    const CDuiString& CComboUI::GetText()
	{
		//loki 增加向父类获取信息 ----------------------begin
	...
	{
        //适配dpi by jason 2019-08-12
        if (m_pManager)
            return m_pManager->GetDPIObj()->Scale(m_szDropBox);
		return m_szDropBox;
	}
	...
	{
        if (m_pManager)
            return m_pManager->GetDPIObj()->Scale(m_rcDropBox);
		return m_rcDropBox;
	}
	...
	{
        if (m_pManager)
            return m_pManager->GetDPIObj()->Scale(m_rcTextPadding);
    ...
    void CComboUI::SetItemSelectedFont(int index)
    {
	...
	{
        if (m_pManager)
            return m_pManager->GetDPIObj()->Scale(m_ListInfo.rcTextPadding);
	...
	void CComboUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
        ...
        if (_tcsicmp(pstrName, _T("dropboxalign")) == 0) {
            if (_tcsstr(pstrValue, _T("left")) != NULL) {
                m_uDropBoxAlign &= DT_RIGHT;
                m_uDropBoxAlign |= DT_LEFT;
            }
            if (_tcsstr(pstrValue, _T("right")) != NULL) {
                m_uDropBoxAlign &= DT_LEFT;
                m_uDropBoxAlign |= DT_RIGHT;
		...
		else if( _tcsicmp(pstrName, _T("dropboxsize")) == 0)
		{
        ...
        else if (_tcsicmp(pstrName, _T("itemselectedfont")) == 0) SetItemSelectedFont(_ttoi(pstrValue));//add by jason 2019-10-21 设置选中字体
            ...
            // itemtext 单行显示 modify by jshuang 2019-09-18
			if( _tcsicmp(pstrValue, _T("true")) == 0 ) m_ListInfo.uTextStyle |= (DT_END_ELLIPSIS | DT_SINGLELINE);
