...
namespace DuiLib
{
	...
	static const TCHAR *strWndClassName = _T("PerryShadowWnd");
	...
	void CShadowUI::Create(CPaintManagerUI* pPaintManager)
	{
        ...
        //styleValue = styleValue & ~WS_DLGFRAME;
...
#pragma warning(default: 4311)
	...
	LRESULT CALLBACK CShadowUI::ParentProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
	{
        ...
        static std::map<HWND, CShadowUI*>& mapRef = GetShadowMap();
        std::map<HWND, CShadowUI*>::iterator it = mapRef.find(hwnd);
        if (it == mapRef.end() || !it->second)
        {
            _ASSERT(0);
            return ::DefWindowProc(hwnd, uMsg, wParam, lParam);
        }

        CShadowUI *pThis = it->second;// GetShadowMap()[hwnd];
...
#pragma warning(default: 4312)
		...
		case WM_NCACTIVATE:
		{
			::SetWindowPos(pThis->m_hWnd, hwnd, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE | SWP_NOACTIVATE | SWP_NOREDRAW | SWP_NOZORDER);
			break;
		}
		case WM_WINDOWPOSCHANGED:
			...
			if (pThis->m_bIsImageMode) {
				SetWindowPos(pThis->m_hWnd, hwnd, WndRect.left - pThis->m_nSize, WndRect.top - pThis->m_nSize, 0, 0, SWP_NOSIZE | SWP_NOACTIVATE | SWP_NOZORDER);
			}
			else {
				SetWindowPos(pThis->m_hWnd, hwnd, WndRect.left + pThis->m_nxOffset - pThis->m_nSize, WndRect.top + pThis->m_nyOffset - pThis->m_nSize, 0, 0, SWP_NOSIZE | SWP_NOACTIVATE | SWP_NOZORDER);
			}
			break;
		case WM_MOVE:
...
#pragma warning(default: 4312)
	...
	bool CShadowUI::SetPosition(int NewXOffset, int NewYOffset)
	{
        ...
        //loki add 
        if (m_nxOffset == NewXOffset && m_nyOffset == NewYOffset)
            return true;
	...
	bool CShadowUI::SetShadowCorner(RECT rcCorner)
	{
        ...
        //loki add 
        if (::EqualRect(&m_rcShadowCorner, &rcCorner))
            return true;
