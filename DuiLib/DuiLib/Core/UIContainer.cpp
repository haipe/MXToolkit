...
namespace DuiLib
{
	...
	//loki 保留个数参数 -----begin
	void CContainerUI::RemoveAll( int reserveCount )
	{
		int it = reserveCount <= 0 ? 0 : reserveCount;
		for( ; m_bAutoDestroy && it < m_items.GetSize(); it++ ) {
		...
		{
			int removeCount = m_items.GetSize() - reserveCount;
			if ( removeCount > 0 )
				m_items.Remove( reserveCount, removeCount );
		}
		NeedUpdate();
	}
	//--------------------- end 
	//loki 取出元素--------------------------begin
	CControlUI* CContainerUI::TakeOutItem( CControlUI* pControl )
	{
		if ( pControl == nullptr )
			return nullptr;

		return TakeOutItemAt( GetItemIndex( pControl ), pControl );
	}

	CControlUI* CContainerUI::TakeOutItemAt( int index, CControlUI* pControl )
	{
		if ( index < 0 )
			return nullptr;

		bool old = m_bAutoDestroy;
		m_bAutoDestroy = false;
		bool ret = RemoveAt( index );
		m_bAutoDestroy = old;

		return ret ? pControl : nullptr;
	}
	//--------------------------------end
	...
	bool CContainerUI::DoPaint(HDC hDC, const RECT& rcPaint, CControlUI* pStopControl)
	{
		...
		if( m_items.GetSize() > 0 ) {
            // modify by jshuang , m_rcInset适配DPI导致位置错误， 暂不适配DPI
            RECT rcInset = m_rcInset; //GetInset(); 
			RECT rc = m_rcItem;
	...
	void CContainerUI::SetFloatPos(int iIndex)
	{
        ...
        if( pControl->IsManualFloat() ) return; //loki add 手动调用SetPos
	...
	{
		CControlUI* pSubControl=NULL;
		if(GetManager()) pSubControl = static_cast<CControlUI*>(GetManager()->FindSubControlByName(this,pstrSubControlName));
		return pSubControl;
	}
