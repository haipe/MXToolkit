#include "StdAfx.h"
#include <zmouse.h>
#include "Utils\ResourceCacheUtil.h"
...
namespace DuiLib {
...
#ifdef _USE_ADAPTER_DPI_
        // 调整DPI资源
        if (pManager != NULL && pManager->GetDPIObj()->GetScale() != 100) {
            CDuiString parentPath;
            parentPath.Format(_T("%d\\"), pManager->GetDPIObj()->GetScale());
            CDuiString sScale;
            sScale.Format(_T("@%d."), pManager->GetDPIObj()->GetScale());

            //2019-4-19 loki 这里只要替换最后一个 后缀名的地方 xxx.xxx 变成 xxx@DPI.xxx
            //sImageName.Replace(_T("."), sScale);  会替换所有的 . 为 @.
            int pos = sImageName.ReverseFind(_T('.'));
            if (pos >= 0)
            {
                int sLength = sImageName.GetLength();
                sScale += sImageName.Right(sLength - pos - 1);
                sImageName = sImageName.Left(pos);

                // 2019-8-8 modify by jshuang
                // 增加一级文件路径， 以DPI命名 如：parent\\xxx@DPI.xxx 变成 parent\\DPI\\xxx@DPI.xxx
                sLength = sImageName.GetLength();
                pos = sImageName.ReverseFind(_T('\\'));
                sImageName = sImageName.Left(pos + 1) + parentPath + sImageName.Right(sLength - pos - 1);
                sImageName += sScale;
            }
        }
#endif // _USE_ADAPTER_DPI_
	...
	void CPaintManagerUI::SetResourceZip(LPCTSTR pStrPath, bool bCachedResourceZip, LPCTSTR password)
	{
        ...
        if (bCachedResourceZip)
            ResourceCacheUtil::GetInstance()->Init();
	}
		...
		case WM_KEYDOWN:
			{
				...
				if( wParam == VK_TAB ) {
					...
					if( m_pFocus && m_pFocus->IsVisible() && m_pFocus->IsEnabled() && _tcsstr(m_pFocus->GetClass(), _T("WkeWebkit")) != NULL ) {
						return false;
					}
		...
		for( int i = 0; i < m_aMessageFilters.GetSize(); i++ ) 
		{
			...
			if( bHandled ) {
				...
				case WM_MOUSEWHEEL:
					...
					break;
                case WM_KEYDOWN:        // 增加Tab切换焦点消息处理 add by jshuang 2019-09-05
                {
                    // Tabbing between controls
                    if (wParam == VK_TAB) {
		...
		case WM_PAINT:
			{
                ...
                // 此处不进行tab切换焦点，会导致焦点设置错误 modify by jshuang 2019-09-05
				//if( m_bFocusNeeded ) {
				//	SetNextTabControl();
				//}
		...
		case WM_MOUSEHOVER:
			{
				...
				//Zero新增 tip提示cnSize参数设置问题---begin
				...
				if( m_hwndTooltip == NULL ) {
					m_hwndTooltip = ::CreateWindowEx(0, TOOLTIPS_CLASS, NULL, WS_POPUP | TTS_NOPREFIX | TTS_ALWAYSTIP, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, m_hWndPaint, NULL, m_hInstance, NULL);//m_hWndPaint绘制窗口句柄更改为桌面，即置顶
	...
	//loki 增加判断是否已经加载xml
    bool CPaintManagerUI::IsAttached()
    {
	...
	bool CPaintManagerUI::InitControls(CControlUI* pControl, CControlUI* pParent /*= NULL*/)
	{
        ...
        // __FindControlFromNameHash操作要在SetManager之前
        // 因为SetManager(*, *, true)会调用DoInit()对控件初始化
        // 如果控件在初始化时去FindControl()查找子控件时才能通过PaintManager找到
        // 例如：CListUI的ListItem被add到CListUI后，将触发ListItem::DoInit()
        // ListItem::DoInit()中再查找子控件将得到正确控件指针
        // @yuegy, #2019/8/2
	...
	void CPaintManagerUI::Term()
	{
        ...
        ResourceCacheUtil::GetInstance()->Release();
    ...
    // 只适配100%、150%、200%比例dpi
    int AdjustDpiValue(int dpi)
    {
        if (dpi <= 96)
            return 96;      //100%
        else if (dpi <= 144)
	...
	void DuiLib::CPaintManagerUI::SetDPI(int iDPI)
	{
		...
		// 窗口不指定xml时，GetRoot会引发奔溃 modify by jshuang 2019-8-21
        if (m_pRoot != NULL) GetRoot()->NeedUpdate();
		::PostMessage(GetPaintWindow(), UIMSG_SET_DPI, 0, 0);
#endif // _FS_ADAPTER_DPI_
	}
    ...
    void DuiLib::CPaintManagerUI::SetAllDPI(HWND hWnd, int iDPI)
    {
        for (int i = 0; i < m_aPreMessages.GetSize(); i++) {
            CPaintManagerUI* pManager = static_cast<CPaintManagerUI*>(m_aPreMessages[i]);
            if (pManager && hWnd == GetParent(pManager->GetPaintWindow()))
                pManager->SetDPI(iDPI);
        }
    }
	...
	void DuiLib::CPaintManagerUI::ResetDPIAssets()
	{
		...
		CStdPtrArray *richEditList = FindSubControlsByClass(GetRoot(), DUI_CTR_RICHEDIT);//richedit名称错误. by jason 2019-8-2
	...
	void CPaintManagerUI::SetFocus(CControlUI* pControl)
	{
		...
		if( m_pFocus != NULL ) 
		{
			...
			event.dwTimestamp = ::GetTickCount();
            m_pFocus->Event(event);
            //loki Edit失去焦点的时候隐藏自己，嵌套死循环。
            CControlUI* focusCtl = m_pFocus;
			m_pFocus = NULL;
			SendNotify(focusCtl, DUI_MSGTYPE_KILLFOCUS);
			//--------------------end
		}
    ...
    void CPaintManagerUI::SetNotFocusNeeded()
    {
        m_bFocusNeeded = false;
    }
	...
	//---loki 增加资源模块id
	void CPaintManagerUI::SetResourceModuleID( int res )
	{
		m_resourceModuleID = res;
	}

	int CPaintManagerUI::GetResourceModuleID() const
	{
		return m_resourceModuleID > 0 ? m_resourceModuleID : -1;
	}
	//------end

	//Zero Add
	void CPaintManagerUI::CleanToolTips()
	{
		if (m_hwndTooltip != NULL) 
		{
			::DestroyWindow(m_hwndTooltip);
