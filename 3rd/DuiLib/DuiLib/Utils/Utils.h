...
namespace DuiLib
{
	...
	inline CDuiString operator+(const CDuiString& left, LPCTSTR right)
	{
		if (right)
		{
			CDuiString temp(left);
			temp.Append(right);
			return temp;
		}
		return left;
	...
	inline CDuiString operator+(LPCTSTR left, const CDuiString& right)
	{
		if (left)
		{
			CDuiString temp(left);
	...
	inline CDuiString operator+(const CDuiString& left, const CDuiString& right)
	{
		return left + right.GetData();
	}

	inline bool operator==(const CDuiString& left, LPCTSTR right)
	{
		return left.Compare(right) == 0;
	}

	inline bool operator==(LPCTSTR left, const CDuiString& right)
	{
		return right == left;
	}

	inline bool operator==(const CDuiString& left, const CDuiString& right)
	{
		return left == right.GetData();
	}
