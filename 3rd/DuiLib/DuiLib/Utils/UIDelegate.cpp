...
//lokiÐÂÔö--------------------begin
void CEventSource::Copy( const CEventSource& src )
{
	Clear();
	for ( int i = 0; i < src.m_aDelegates.GetSize(); i++ ) {
		CDelegateBase* pObject = static_cast< CDelegateBase* >( src.m_aDelegates[i] );
		if ( pObject )m_aDelegates.Add( pObject->Copy() );
	}
