...
namespace DuiLib
{
	...
	static const TCHAR *strWndClassName = _T("DuiShadowWnd");
	...
	//CStdValArray CWndShadow::s_ShadowArray(sizeof(HwndShadow), 10);
...
// 		}
	...
	void CWndShadow::Create(CPaintManagerUI* pm)
	{
		// Do nothing if the system does not support layered windows
        if (NULL == s_UpdateLayeredWindow && pm)
        {
            Initialize(pm->GetInstance());
        }
        ...
        HWND hParentWnd = pm->GetPaintWindow();
        m_pManager = pm;
...
#pragma warning(default: 4311)
    ...
    {
        CWndShadow *pThis = CWndShadow::GetShadowMap()[hwnd];
		...
		{
		case WM_MOVE:
		if((pThis->m_Status & SS_VISABLE) && (pThis->m_Status & SS_ENABLED))
			{
				RECT WndRect;
...
#pragma warning(default: 4312)
    ...
    std::map<HWND, CWndShadow *>& CWndShadow::GetShadowMap()
    {
...
// 	}
	...
	void CWndShadow::Update(HWND hParent)
    {
        ...
        if (m_pManager && !m_pImageInfo)
            m_pImageInfo = m_pManager->GetImageEx((LPCTSTR)m_sShadowImage, NULL, 0);
...
// 	}
    ...
    bool CWndShadow::SetImage(LPCTSTR szImage)
    {
        if (szImage == NULL)
            return false;

        //loki add 
        if (m_sShadowImage == szImage)
            return true;
    ...
    bool CWndShadow::CopyShadow(CWndShadow* pShadow)
    {
        if (m_bIsImageMode) {
            pShadow->SetImage(m_sShadowImage);
            pShadow->SetShadowCorner(m_rcCorner);
    ...
    void CWndShadow::ShowShadow(bool bShow)
    {
        m_bIsShowShadow = bShow;
    }

    bool CWndShadow::IsShowShadow() const
    {
        return m_bIsShowShadow;
    }
    
    void CWndShadow::DisableShadow(bool bDisable)
    {
    ...
    bool CWndShadow::IsDisableShadow() const 
    {
