...
namespace DuiLib
{
	...
	LRESULT CEditWnd::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
	{
        ...
        else if (uMsg == WM_LBUTTONDOWN)
        {
            ...
            if (::IsWindow(m_hWnd) && ::IsWindowVisible(m_hWnd))
            {
                ...
                {
                    if (m_pOwner && !m_pOwner->IsReadOnly() && m_pOwner->IsEnabled())
                        TouchKeyboardUtil::ShowTouchKeyboard(GetHWND());
                }
            }
            bHandled = FALSE;
        }
			...
			if( IsEnabled() ) {
				...
				else if( m_pWindow != NULL )
				{
					if (!m_bAutoSelAll) {
						...
						pt.x -= m_rcItem.left + m_pManager->GetDPIObj()->Scale(m_rcTextPadding.left);//add by jason 2019-10-21 适配dpi
						pt.y -= m_rcItem.top + m_pManager->GetDPIObj()->Scale(m_rcTextPadding.top);
	...
	void CEditUI::PaintText(HDC hDC)
	{
		...
		RECT rc = m_rcItem;
		rc.left += m_pManager->GetDPIObj()->Scale(m_rcTextPadding.left);//add by jason 2019-10-21 适配dpi
