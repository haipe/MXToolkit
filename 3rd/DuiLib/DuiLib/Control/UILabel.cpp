...
namespace DuiLib
{
    ...
    // 设置焦点字体 add by jshuang 2019-8-21
    void CLabelUI::SetFocusFont(int iFont)
    {
        m_iFocusFont = iFont;
        m_bNeedEstimateSize = true;
        Invalidate();
    ...
    void CLabelUI::SetvValignCenter(bool bCenter)
    {
        m_bTextCenter = bCenter;
    }
	...
	SIZE CLabelUI::EstimateSize(SIZE szAvailable)
	{
		...
		if (m_bNeedEstimateSize) {
            int iFont = m_iFont;
            if (IsFocused() && m_iFocusFont != -1)
                iFont = m_iFocusFont;
			CDuiString sText = GetText();
			if (!sText.IsEmpty())	//文本不为空时才不需要重新计算尺寸 add by jason 2018-12-05 
				m_bNeedEstimateSize = false;

			m_szAvailableLast = szAvailable;
			m_cxyFixedLast = m_pManager->GetDPIObj()->Scale(m_cxyFixed);
			if ((m_uTextStyle & DT_SINGLELINE) != 0) {
				if (m_cxyFixedLast.cy == 0) {
					m_cxyFixedLast.cy = m_pManager->GetFontInfo(iFont)->tm.tmHeight + 8;
    ...
    //Zero add in CTextUI, Zero move to here
    CDuiRect CLabelUI::GetTextRect(SIZE szAvailable)
    {
        ...
        if (m_bShowHtml) {
            int nLinks = 0;
            CRenderEngine::DrawHtmlText(m_pManager->GetPaintDC(), m_pManager, rcText, m_sText, m_dwTextColor, NULL, NULL, nLinks, m_iFont, DT_CALCRECT | m_uTextStyle);
        }
        else {
	...
	void CLabelUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
        ...
        else if (_tcsicmp(pstrName, _T("textcenter")) == 0) SetvValignCenter(_tcsicmp(pstrValue, _T("true")) == 0);
		else if( _tcsicmp(pstrName, _T("font")) == 0 ) SetFont(_ttoi(pstrValue));
        else if (_tcsicmp(pstrName, _T("focusfont")) == 0) SetFocusFont(_ttoi(pstrValue));
	...
	void CLabelUI::PaintText(HDC hDC)
	{
        ...
        int iFont = m_iFont;
        // IsFocused时使用焦点字体 modify by jshuang 2019-8-21
        if (IsFocused() && m_iFocusFont != -1) 
            iFont = m_iFocusFont;
