...
public:
	...
	//取消原来的鼠标跟踪  loki---------------begin
	//避免菜单无法弹出  loki 
	void SetTMEEvent( HWND hwnd, DWORD flag );
	//-------------------------------------end

	void GetMonitorWorkArea( int &nWidth, int &nHeight, POINT &pt );
	void GetMonitorWorkArea( CDuiRect& area, HWND wnd );
