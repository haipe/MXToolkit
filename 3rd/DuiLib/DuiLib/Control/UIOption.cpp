...
namespace DuiLib
{
	...
	COptionUI::COptionUI() 
		: m_bSelected(false)
		, m_iSelectedFont(-1)//by jason 2018-11-15 选中的字体大小初始化
		...
		if( m_pManager != NULL ) {
			if( !m_sGroupName.IsEmpty() ) {
				if( m_bSelected ) {
					...
					if(bMsg) {
						m_pManager->SendNotify(this, DUI_MSGTYPE_SELECTCHANGED, bSelected);
					}
				}
				else
				{
					//loki  ---------------增加发送失去选择的消息 begin
					if (bMsg) m_pManager->SendNotify(this, DUI_MSGTYPE_UNSELECTED, bSelected);
					//--------------------------------------------end
				}
			}
			else {
				if(bMsg) {
                    m_pManager->SendNotify(this, m_bSelected ? DUI_MSGTYPE_SELECTCHANGED : DUI_MSGTYPE_UNSELECTED, bSelected);
				}
			}
		}
	...
	//begin 设置选中不可用状态下图片 add by zuojx 2018-08-16
	...
	//end 设置选中不可用状态下图片 add by zuojx 2018-08-16
	...
	void COptionUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		...
		else if( _tcsicmp(pstrName, _T("selecteddisabledimage")) == 0 ) SetSelectedDisabledImage(pstrValue);//设置选中不可用状态下图片 add by zuojx 2018-08-16
	...
	void COptionUI::PaintStatusImage(HDC hDC)
	{
		if(IsSelected()) {
			...
			//绘制选中不可用状态下图片 add by zuojx 2018-08-16
			else if ( !IsEnabled() && !m_sSelectedDisabledImage.IsEmpty() ) {
	...
	void COptionUI::PaintText(HDC hDC)
	{
		if( (m_uButtonState & UISTATE_SELECTED) != 0 )
		{
			...
			int iFont = GetFont();
			if(GetSelectedFont() >= 0) { //changed by liuzheng  不指定selectfont时 默认使用font
