...
namespace DuiLib {
	...
	bool CListUI::RemoveAt(int iIndex)
	{
		...
		// 从选中容易中移除条目 2018.10.24 By Martin
	...
	//loki 保留个数参数 -----begin 2018-02-25
	void CListUI::RemoveAll( int reserveCount )
	{
		if ( reserveCount <= 0 )	//删除所有
		{
			m_iCurSel = -1;
			m_iExpandedItem = -1;
			m_aSelItems.Empty();  //Martin By 2018.09.29
		}
		else
		{
			if (m_iCurSel >= reserveCount)//当前选中超过保留数，将当前选中置为最后一个，否则不变 add by jason 2018-08-16
			{
				int iSel = reserveCount - 1;
				m_iCurSel = -1;
				SelectItem( FindSelectable( iSel, false ) );
			}
		}

		m_pList->RemoveAll( reserveCount );
	}
	//-----------------------end
	...
	bool CListUI::SelectItem(int iIndex, bool bTakeFocus, bool bMsg)
	{
		// 2018.08.25 By Martin
		...
		IListItemUI* pListItem = static_cast<IListItemUI*>(pControl->GetInterface(_T("ListItem")));
		m_iCurSel = iIndex; // 2018.08.25 By Martin
		...
		if( m_pManager != NULL && iLastSel != m_iCurSel && bMsg ) {
			m_pManager->SendNotify(this, DUI_MSGTYPE_ITEMSELECT, iIndex, iLastSel);   // 2018.08.15 By Martin
		}
    ...
    // 增加选中字体设置 add by jshuang 2019-8-21
	void CListUI::SetSelectedItemFont(int iFont)
	{
		m_ListInfo.nSelectedFont = iFont;
		Invalidate();
	}
	...
	void CListUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
        ...
        else if (_tcsicmp(pstrName, _T("itemselectedfont")) == 0) m_ListInfo.nSelectedFont = _ttoi(pstrValue);
	...
	SIZE CListHeaderUI::EstimateSize(SIZE szAvailable)
	{
        ...
        // DPI 适配 modify by jshuang 2019-8-21
        if (m_pManager != NULL)
            return m_pManager->GetDPIObj()->Scale(cXY);
	...
	void CListHeaderUI::SetPos(RECT rc, bool bNeedInvalidate)
	{
        ...
        // GetWidth()获取的宽度错误导致CListHeaderItemUI计算位置错误 -- modify by jshuang 2019-8-19
		int nHeaderWidth = GetFixedWidth();
	...
	{
        // 适配DPI modify by jshuang 2019-8-21
        LONG cx = m_pManager->GetDPIObj()->Scale(m_cxyFixed.cx);
		if( m_cxyFixed.cy == 0 ) return CDuiSize(cx, m_pManager->GetDefaultFontInfo()->tm.tmHeight + 14);
	...
	{
		//if( !IsEnabled() ) return false; //bug24667,其他端主讲设置背景颜色，本端背景颜色选择下拉框选项未变化
	...
	SIZE CListLabelElementUI::EstimateSize(SIZE szAvailable)
	{
        ...
        if (pInfo) {
            iFont = pInfo->nFont;
            if (IsSelected())
                iFont = pInfo->nSelectedFont;
        }
        if (pInfo && pInfo->m_cxyListFixed.cy > 0)  // modify by jshaung 2019-09-30
        {
            cXY.cy = pInfo->m_cxyListFixed.cy;
        }
		else if( cXY.cy == 0 && m_pManager != NULL ) {
			cXY.cy = m_pManager->GetFontInfo(iFont)->tm.tmHeight + 12;
        ...
        // DPI适配 modify by jshuang 2019-8-21
        if (m_pManager != NULL)
            return m_pManager->GetDPIObj()->Scale(cXY);
	...
	void CListLabelElementUI::DrawItemText(HDC hDC, const RECT& rcItem)
	{
        ...
        int iFont = pInfo->nFont;
		if( (m_uButtonState & UISTATE_HOT) != 0 ) {
			iTextColor = pInfo->dwHotTextColor;
		}
		if( IsSelected() ) {
			iTextColor = pInfo->dwSelectedTextColor;
            iFont = pInfo->nSelectedFont;  // 选中时使用选中字体 modify by jshuang 2019-8-21
		}
		if( !IsEnabled() ) {
		...
		if( pInfo->bShowHtml )
			...
			NULL, NULL, nLinks, iFont, pInfo->uTextStyle);
		else
			CRenderEngine::DrawText(hDC, m_pManager, rcText, sText, iTextColor, \
                iFont, pInfo->uTextStyle);
	}
	...
	LPCTSTR CListTextElementUI::GetText(int iIndex)
	{
		...
		{
			//loki 增加多语言获取缓存判断
			if ( !m_pManager || !IsResourceText()  )//每次|| m_nLanguageIdentification == CResourceManager::GetInstance()->GetLanguage()
				return pText->GetData();

			int resourceId = m_pManager->GetResourceModuleID();
			if ( resourceId <= 0 )
				return pText->GetData();

			*pText = CResourceManager::GetInstance()->GetText( m_sName, resourceId );
			if ( IsResourceTooltip() )
				m_sToolTip = *pText;
	...
	SIZE CListTextElementUI::EstimateSize(SIZE szAvailable)
	{
		...
		SIZE cXY = m_cxyFixed;
        if (pInfo && pInfo->m_cxyListFixed.cy > 0)  // modify by jShuang 2019-09-30
        {
            cXY.cy = pInfo->m_cxyListFixed.cy;
        }
        else if( cXY.cy == 0 && m_pManager != NULL ) {
			cXY.cy = m_pManager->GetFontInfo(iFont)->tm.tmHeight + 8;
	...
	void CListTextElementUI::DrawItemText(HDC hDC, const RECT& rcItem)
	{
		...
		for( int i = 0; i < pInfo->nColumns; i++ )
		{
			//-----------对调顺序，实现可以及时修改当前的样式 loki 2018-02-23
			CDuiString strText;
	...
	void CListContainerElementUI::DoEvent(TEventUI& event)
	{
        ...
        // UIEVENT_DBLCLICK 交由 Owner处理 modify by jshuang
 		//if( event.Type == UIEVENT_DBLCLICK )
 		//{
 		//	if( IsEnabled() ) {
 		//		Activate();
