...
#include "Utils\TouchKeyboardUtil.h"
...
err:
	...
	IMPLEMENT_DUICONTROL(CRichEditUI)
        ...
        m_iTipFont(-1), m_dwTipValueColor(0xFFBAC0C5), m_uTipValueAlign(DT_SINGLELINE | DT_LEFT)
	{
		m_bMenu = false;       //Martin By 2018.11.08
#ifndef _UNICODE
	...
	void CRichEditUI::SetEnableMenu( bool bMenu )
	{
		m_bMenu = bMenu;
	}
	...
	//解决win10获取密码为密文的bug 
	CDuiString CRichEditUI::GetRawText() const
	{
		bool bIsPassword = (m_lTwhStyle & ES_PASSWORD);
		TCHAR chPassword;

		if (bIsPassword)
		{
			m_pTwh->TxGetPasswordChar(&chPassword);
			if (chPassword == 0)
			{
				chPassword = _T('*');
			}
			m_pTwh->SetPasswordChar(0);
		}
		...
		if (bIsPassword)
		{
			m_pTwh->SetPasswordChar(chPassword);
		}
	...
	//by jason 2018-06-02 , 修正当文本内容过长时，返回为乱码的bug
	//const CDuiString& CRichEditUI::GetText()
	...
	void CRichEditUI::SetPos(RECT rc, bool bNeedInvalidate)
	{
		...
		if( m_pTwh != NULL ) {
            ...
            //适配dpi add by jason 2019-10-18
			rcScrollTextView.left += m_pManager->GetDPIObj()->Scale(m_rcTextPadding.left);
			rcScrollTextView.right -= m_pManager->GetDPIObj()->Scale(m_rcTextPadding.right);
			rcScrollTextView.top += m_pManager->GetDPIObj()->Scale(m_rcTextPadding.top);
			rcScrollTextView.bottom -= m_pManager->GetDPIObj()->Scale(m_rcTextPadding.bottom);
			RECT rcText = rc;
			rcText.left += m_pManager->GetDPIObj()->Scale(m_rcTextPadding.left);
			rcText.right -= m_pManager->GetDPIObj()->Scale(m_rcTextPadding.right);
			rcText.top += m_pManager->GetDPIObj()->Scale(m_rcTextPadding.top);
			rcText.bottom -= m_pManager->GetDPIObj()->Scale(m_rcTextPadding.bottom);
		...
		if( m_pVerticalScrollBar != NULL && m_pVerticalScrollBar->IsVisible() ) {
			...
			TxSendMessage(WM_VSCROLL, SB_LINEDOWN, 0L, 0);//拉伸聊天框滚动条长度变化，显示最新文本内容
		}
	...
	bool CRichEditUI::DoPaint(HDC hDC, const RECT& rcPaint, CControlUI* pStopControl)
	{
		...
		if((sDrawText.IsEmpty() && !m_bFocused)
			|| (m_bKeepShowTip && sDrawText.IsEmpty())) //登录框提示文字显示特例
		{
			...
			RECT rc = m_rcItem;
            //适配dpi add by jason 2019-10-18
			rc.left += m_pManager->GetDPIObj()->Scale(m_rcTextPadding.left);
            rc.right -= m_pManager->GetDPIObj()->Scale(m_rcTextPadding.right);
            // tip文字默认上下居中，不根据textpadding 计算 modify by jshuang 
            //rc.top += m_pManager->GetDPIObj()->Scale(m_rcTextPadding.top);
            //rc.bottom -= m_pManager->GetDPIObj()->Scale(m_rcTextPadding.bottom);
            //if (IsMultiLine())
            //{
            //    uTextAlign |= DT_TOP;
            //}
            ...
            int iFont = m_iFont;
            if (m_iTipFont != -1)
                iFont = m_iTipFont;  // 使用提示字体， modify by jshuang 2019-8-22
			CRenderEngine::DrawText(hDC, m_pManager, rc, sTipValue, dwTextColor, iFont, uTextAlign);
		}
		return true;
	}
	...
	void CRichEditUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
        ...
        else if (_tcscmp(pstrName, _T("keepshowtip")) == 0) {
            SetKeepShowTip(_tcscmp(pstrValue, _T("true")) == 0);
        }
        ...
        else if (_tcscmp(pstrName, _T("menu")) == 0) {  // add by jshuang 2019-09-27
            SetEnableMenu(_tcscmp(pstrValue, _T("true")) == 0);
        }
	...
	LRESULT CRichEditUI::MessageHandler(UINT uMsg, WPARAM wParam, LPARAM lParam, bool& bHandled)
	{
		...
		if( (uMsg >= WM_MOUSEFIRST && uMsg <= WM_MOUSELAST) || uMsg == WM_SETCURSOR ) {
			...
			if( uMsg == WM_SETCURSOR ) bWasHandled = false;
				...
				//解决点击子窗口RichEdit时，焦点被父控件获取的问题。 2016.11.15 By Martin
				if (!GetManager()->IsNoActivate()) ::SetFocus(GetManager()->GetPaintWindow());

                //2019-04-24 by jason 点击输入框时，显示触摸键盘
                if (uMsg == WM_LBUTTONDOWN && !IsReadOnly() && IsEnabled())
                    TouchKeyboardUtil::ShowTouchKeyboard(GetManager()->GetPaintWindow());
			}
		}
#ifdef _UNICODE
			...
			if (m_bMenu)
			{
				...
				// 字体写错 2018.10.22 By Martin
				AppendMenu( hPopMenu, 0, ID_RICH_PASTE, _T( "粘贴(&V)" ) );  
		...
		LRESULT lResult = 0;
        HRESULT Hr = S_FALSE;
        // 若 wantTab == false; 则 wParam == VK_TAB 时不发送TextChange消息; modify by jshuang 2019-10-30
        if (!(uMsg >= WM_KEYFIRST && uMsg <= WM_KEYLAST && wParam == VK_TAB) || IsWantTab())
            Hr = TxSendMessage(uMsg, wParam, lParam, &lResult);
