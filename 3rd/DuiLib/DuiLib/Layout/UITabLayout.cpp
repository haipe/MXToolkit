...
namespace DuiLib
{
    ...
    void CTabLayoutUI::SetVisible(bool bVisible /*= true*/)
    {
        ...
        for (int it = 0; it < m_items.GetSize(); it++) {
            ...
            if (it == m_iCurSel)
                GetItemAt(it)->SetVisible(IsVisible());
        }
    }

    bool CTabLayoutUI::SelectItem(int iIndex, bool bTakeFocus)
	{
		...
		{
			if( it == iIndex ) {
				GetItemAt(it)->SetVisible(IsVisible());//loki add tab 隐藏的时候 选择 2018-08-24
		...
		if( m_pManager != NULL ) {
			if (bTakeFocus) {  //控制焦点的切换Martin By 2018.09.29
