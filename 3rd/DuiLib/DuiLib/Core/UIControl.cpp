...
namespace DuiLib {
	IMPLEMENT_DUICONTROL(CControlUI)
		...
		m_bFloat(false),
        m_bManualFloat(false),
		m_uFloatAlign(0),
		m_bSetPos(false),
		m_bDragEnabled(false),
		m_bDropEnabled(false),
		m_bResourceText(false),
		m_nLanguageIdentification(-1),//loki 增加多语言标识
		m_chShortcut('\0'),
		m_pTag(NULL),
		m_dwBackColor(0),
		m_dwBackColor2(0),
	...
	void CControlUI::SetName(LPCTSTR pstrName)
	{
        ...
        bool isResourceTooltip = IsResourceTooltip();
        if (isResourceTooltip && m_sName == m_sResourceTooltip)
            m_sResourceTooltip = pstrName;

        m_sName = pstrName;
        if (IsResourceText() || isResourceTooltip)
        {
            m_nLanguageIdentification = -1;
            NeedUpdate();
        }
		//----------------------end
	}
	...
	//loki 增加多语言获取缓存判断
	const CDuiString& CControlUI::GetText()
	{
		if ( !m_pManager || !IsResourceText() || CResourceManager::GetInstance()->GetLanguage() == m_nLanguageIdentification )
			return m_sText;
		...
		LPCTSTR text = CResourceManager::GetInstance()->GetText( m_sName, resourceId );
		if ( text )
		{
			SetText( text );
			if ( IsResourceTooltip() )
				m_sToolTip = m_sText;
			m_nLanguageIdentification = CResourceManager::GetInstance()->GetLanguage();
		}
	...
	//loki增加多语言tooltip控制---------------------
	bool CControlUI::IsResourceTooltip() const
	{
		return !m_sResourceTooltip.IsEmpty();
	}
	
    const DuiLib::CDuiString& CControlUI::GetResourceTooptip() const
    {
        return m_sResourceTooltip;
    }

    void CControlUI::SetResourceTooptip(LPCTSTR pstrName)
	{
		if (pstrName && m_sResourceTooltip == pstrName)
            return;
	...
	//loki 增加多语言获取缓存判断
    const CDuiString& CControlUI::GetToolTip()
    {
        if (!m_pManager || !IsResourceTooltip() || CResourceManager::GetInstance()->GetLanguage() == m_nLanguageIdentification)
            return m_sToolTip;
        ...
        LPCTSTR text = CResourceManager::GetInstance()->GetText(m_sResourceTooltip, resourceId);
        if (text)
        {
            SetToolTip(text);
            if (IsResourceText())
    ...
    ////loki add 手动调用SetPos  xml manualfloat="true" 属性 默认 false ------------
    bool CControlUI::IsManualFloat() const
    {
        return m_bManualFloat;
    }

    void CControlUI::SetManualFloat(bool bManual /*= false*/)
    {
        if (m_bManualFloat == bManual) 
            return;

        m_bManualFloat = bManual;
        NeedParentUpdate();
    }
    //end------
	...
	void CControlUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
        ...
        else if (_tcsicmp(pstrName, _T("resourcetooltip")) == 0) SetResourceTooptip(_tcsicmp(pstrValue, _T("true")) == 0 ? GetName(): pstrValue);//loki增加tooltip多语言控制
		...
		else if( _tcsicmp(pstrName, _T("manualfloat")) == 0 ) SetManualFloat(_tcsicmp(pstrValue, _T("true")) == 0);
