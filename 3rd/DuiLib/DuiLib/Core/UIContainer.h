...
namespace DuiLib {
	...
	public:
		//loki新增，取出，不删除 ---begin
		virtual CControlUI* TakeOutItem( CControlUI* pControl ) = 0;
		virtual CControlUI* TakeOutItemAt( int index, CControlUI* pControl ) = 0;
		//-------------------------end
		...
		virtual void RemoveAll( int reserveCount = 0 ) = 0;
		//-----------------------end
	};
	...
	public:
		...
		//loki 取出控件,用于将控件移到其他PaintManager
		CControlUI* TakeOutItem( CControlUI* pControl );
		CControlUI* TakeOutItemAt( int index, CControlUI* pControl = nullptr );
		//-----------------------end
