...
namespace DuiLib {
	//by jason 2019-03-05 屏蔽掉下面这行代码，此处会导致内存泄漏，具体原因未知
    //const int& CResourceManager::m_currentLanguage = CResourceManager::GetInstance()->GetLanguage(); 
...
// 
	...
	LPCTSTR CResourceManager::GetText( const CDuiString& name, int resource )
{
	if ( m_pQuerypInterface )
		return m_pQuerypInterface->QueryControlText( name, resource );
