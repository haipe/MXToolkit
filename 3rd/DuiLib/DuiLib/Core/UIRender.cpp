...
namespace DuiLib {
	...
	TImageInfo* CRenderEngine::LoadImage(STRINGorID bitmap, LPCTSTR type, DWORD mask, HINSTANCE instance)
	{
		...
		do 
        {
			if( type == NULL )
            {
                ResourceCacheUtil::GetInstance()->FindImage(bitmap.m_lpstr, pData, dwSize);
			}
			else 
            {
                ResourceCacheUtil::GetInstance()->FindImageFromInstance(instance, type, bitmap.m_lpstr, pData, dwSize);
			}
		} while (0);

		if (!pData)
		{
            ResourceCacheUtil::GetInstance()->FindImageFromLocal(bitmap.m_lpstr, pData, dwSize);
		}
	...
	Gdiplus::Image* CRenderEngine::GdiplusLoadImage(LPCTSTR pstrPath)
	{
		LPBYTE pData = NULL;
		DWORD dwSize = 0;
        ResourceCacheUtil::GetInstance()->FindImage(pstrPath, pData, dwSize);

		if (!pData)
        {
            ResourceCacheUtil::GetInstance()->FindImageFromLocal(pstrPath, pData, dwSize);
		}
