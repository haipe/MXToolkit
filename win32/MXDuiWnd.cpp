...
namespace mxtoolkit
{
    ...
    HICON MXDuiWnd::s_smalIcon = 0;
    HICON MXDuiWnd::s_bigIcon = 0;

    MXDuiWnd::MXDuiWnd(const TCHAR* xml)
        : m_xmlFile(xml)
    {
    }
    ...
    void MXDuiWnd::ShowInTask(HWND wnd, bool show)
    {
    ...
    bool MXDuiWnd::CheckWindowStyle(HWND wnd, DWORD style)
    {
    ...
    void MXDuiWnd::SetNotify(DuiLib::INotifyUI* notify)
    {
        if (notify == NULL)
            return;
    ...
    void MXDuiWnd::SetBuilderCallback(DuiLib::IDialogBuilderCallback* callback)
    {
        if (callback == NULL)
            return;
    ...
    void MXDuiWnd::ShowShadow(bool bShow)
    {
    ...
    void MXDuiWnd::CenterWindow(int width, int height)
    {
        POINT pt = { 0,0 };
        //如果父窗口隐藏了
    ...
    void MXDuiWnd::SetWindowText(LPCTSTR text)
    {
    ...
    void MXDuiWnd::PopWindow(DWORD addFlag, DWORD removeFlag)
    {
    ...
    DWORD MXDuiWnd::ModalWindow(DWORD addFlag, DWORD removeFlag)
    {
        int   nStyleOffset = GWL_STYLE;
        DWORD dwStyle = ::GetWindowLong(m_hWnd, nStyleOffset);
        DWORD dwNewStyle = (dwStyle & ~removeFlag) | addFlag;
        ...
        MXDuiWnd::ShowShadow(true);
        return DuiLib::CWindowWnd::ShowModal();
    }

    void MXDuiWnd::TileWindow(DWORD addFlag, DWORD removeFlag)
    {
        int   nStyleOffset = GWL_STYLE;
        DWORD dwStyle = ::GetWindowLong(m_hWnd, nStyleOffset);
        DWORD dwNewStyle = (dwStyle & ~removeFlag) | addFlag;
    ...
    void MXDuiWnd::InitWndAbilityManager()
    {
        //根据默认名称查找
    ...
    LRESULT MXDuiWnd::OnTimer(WPARAM wParam, LPARAM lParam, BOOL* bHandled)
    {
        *bHandled = FALSE;
        return 0;
    }

    LRESULT MXDuiWnd::OnCreate(WPARAM wParam, LPARAM lParam, BOOL* bHandled)
    {
        if (m_xmlFile.empty())
            return 0;
    ...
    LRESULT MXDuiWnd::OnClose(WPARAM wParam, LPARAM lParam, BOOL* bHandled)
    {
        *bHandled = FALSE;
        return 0;
    }

    LRESULT MXDuiWnd::OnDestroy(WPARAM wParam, LPARAM lParam, BOOL* bHandled)
    {
        *bHandled = FALSE;
        return 0;
    }

    LRESULT MXDuiWnd::OnNcActivate(WPARAM wParam, LPARAM lParam, BOOL* bHandled)
    {
        if (::IsIconic(*this))
            *bHandled = FALSE;
    ...
    LRESULT MXDuiWnd::OnNcCalcSize(WPARAM wParam, LPARAM lParam, BOOL* bHandled)
    {
        LPRECT pRect = NULL;
        if (wParam == TRUE)
        {
    ...
    LRESULT MXDuiWnd::OnNcPaint(WPARAM wParam, LPARAM lParam, BOOL* bHandled)
    {
        return 0;
    }

    LRESULT MXDuiWnd::OnNcHitTest(WPARAM wParam, LPARAM lParam, BOOL* bHandled)
    {
        POINT pt;
    ...
    LRESULT MXDuiWnd::OnNCLButtonDBClick(WPARAM wParam, LPARAM lParam, BOOL* bHandled)
    {
        *bHandled = false;
    ...
    LRESULT MXDuiWnd::OnSize(WPARAM wParam, LPARAM lParam, BOOL* bHandled)
    {
    ...
    LRESULT MXDuiWnd::OnGetMinMaxInfo(WPARAM wParam, LPARAM lParam, BOOL* bHandled)
    {
    ...
    LRESULT MXDuiWnd::OnSysCommand(WPARAM wParam, LPARAM lParam, BOOL* bHandled)
    {
    ...
    LRESULT MXDuiWnd::OnKeyDown(WPARAM wParam, LPARAM lParam, BOOL* bHandled)
    {
        *bHandled = FALSE;
        return 0;
    }
